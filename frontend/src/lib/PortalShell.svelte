<script lang="ts">
 ...
 const closeNavigationOnBackdrop = (event: MouseEvent) => { if (event.target === event.currentTarget) navigationOpen = false; };
</script>

<div class="portal-shell min-h-screen bg-[#f7f9fc] text-slate-900">
 <aside class="fixed inset-y-0 left-0 z-40 hidden w-20 flex-col border-r border-slate-200 bg-white lg:flex">
  <div class="flex h-16 items-center justify-center border-b border-slate-200"><a href="/dashboard" class="flex h-9 w-9 items-center justify-center rounded-xl bg-slate-950 text-xs font-black text-teal-300 shadow-lg shadow-slate-900/10" aria-label="CyberAsk home">CA</a></div>
  <nav class="flex flex-1 flex-col items-center gap-2 px-2 py-5" aria-label="Workspace navigation">{#each primary as item}<a href={item.href} class="rail-link" class:active={isActive(item.href)} title={item.label} aria-label={item.label}><Icon name={item.icon} /></a>{/each}<div class="my-3 h-px w-8 bg-slate-200"></div>{#each manage as item}<a href={item.href} class="rail-link" class:active={isActive(item.href)} title={item.label} aria-label={item.label}><Icon name={item.icon} /></a>{/each}</nav>
...
<style>
 ...
 :global(.tile-icon) { display:flex; height:2rem; width:2rem; align-items:center; justify-content:center; border-radius:.6rem; }
</style>
