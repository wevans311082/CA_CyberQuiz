...
<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
	<div class="rounded-[1.75rem] border border-slate-200 bg-white shadow-[0_30px_80px_rgba(15,23,42,0.18)] text-slate-900 w-full max-w-2xl flex flex-col gap-6 p-8">
		<div class="flex items-center justify-between">
		<h2 class="text-xl font-semibold text-slate-900">{$t('security_settings.totp_setup.totp_setup')}</h2>
			<button onclick={() => { totp_data = undefined; }} class="rounded-full border border-slate-300 px-4 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50 transition-colors">
				{$t('words.close')}
			</button>
		</div>
